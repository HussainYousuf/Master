<?php 
    ...
    if(isset($_POST["submit"])){
        $ids =$_POST["list"];
        foreach($ids as $id){
            mysqli_query($conn,"delete from user_community where user_id= {$_SESSION['id']} and community_id = $id") or die(mysqli_error($conn));
        }
    }

    if($_SESSION["type"] === 'user') $res = mysqli_query($conn,"select community_id from user_community where user_id = {$_SESSION['id']}") or die(mysqli_error($conn));
