...
map ::
  ...
  -> List a
  -> List b
map func = foldRight (\x xs -> func x :. xs) Nil
...
filter ::
  ...
  -> List a
  -> List a
filter func = foldRight (\x xs -> if func x then x:.xs else xs) Nil
...
find ::
  ...
  -> List a
  -> Optional a
find func = foldRight (\a b -> if func a then Full a else b) Empty
