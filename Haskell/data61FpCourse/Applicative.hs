...
instance Applicative ExactlyOne where
  ...
  (<*>) ::
    ...
    -> ExactlyOne a
    -> ExactlyOne b
  (<*>) (ExactlyOne func) = mapExactlyOne func 
...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) funcs as = flatten $ map (\f -> map f as) funcs
...
lift3 ::
  ...
  -> k c
  -> k d
lift3 func ka kb kc = func <$> ka <*> kb <*> kc
...
lift4 ::
  ...
  -> k d
  -> k e
lift4 func ka kb kc kd = func <$> ka <*> kb <*> kc <*> kd
...
filtering func as = lift2 (\as' bs -> foldRight (\x xs-> if snd x then fst x:.xs else xs) Nil (zip as' bs)) (pure as) (sequence (func <$> as))
-- filtering func as = as <$ (sequence (func <$> as))
-- map (\f -> if )
...
-- K list a <*
-- K list bool
-- filtering func as = sequence (func <$> as)  
